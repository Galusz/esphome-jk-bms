...
void JkBmsBle::decode_jk02_cell_info_(const std::vector<uint8_t> &data) {
  ...
  if (frame_version == FRAME_VERSION_JK02_32S) {
    ...
    this->publish_state_(this->charging_status_sensor_, (unsigned char) data[248 + offset]);
  }


      ESP_LOGI(TAG, "Hex_All  %s", format_hex_pretty(&data.front() + 248+offset, data.size() - 248 - offset).c_str());
